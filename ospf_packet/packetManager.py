...
def handle_hello_packet(packet, router, interface):
    ip_packet = packet[IP]
...
def handle_dd_packet(packet, router, interface):
    ip_packet = packet[IP]
    src_ip, dst_ip = ip_packet.src, ip_packet.dst
...
def handle_lsr_packets(packet, router, interface):
    ip_packet = packet[IP]
    src_ip, dst_ip = ip_packet.src, ip_packet.dst
...
def handle_lsu_packets(packet, router, interface):
    ip_packet = packet[IP]
    src_ip, dst_ip = ip_packet.src, ip_packet.dst
    ...
    for lsa in lsu_packet.lsa_list:
        ...
        pass
        # (4) 如果 LSA 的 LS 时限等于 MaxAge，而且路由器的连接状态数据库中没有该LSA的实例，而且路由器的邻居都不处于Exchange或Loading状态
...
def handle_lsAck_packets(packet, router, interface):
    ip_packet = packet[IP]
    src_ip, dst_ip = ip_packet.src, ip_packet.dst
